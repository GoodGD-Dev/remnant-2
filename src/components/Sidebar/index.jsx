...
const MenuItem = ({
  ...
  if (item.subOptions) {
    return (
      <Dropdown className="mb-2" show={isOpen}>
        {" "}
        {/* Controla a visibilidade do dropdown */}
        <Dropdown.Toggle
          className={`dropdown-toggle-custom ${
            activeIndex === index ? "active-item" : ""
          }`} // Adiciona classe ativa se o item for o selecionado
    ...
    <Nav.Item>
      <Nav.Link
        ...
        className={`nav-link-custom ${
          activeIndex === index ? "active-item" : ""
        }`} // Adiciona classe ativa se for o item selecionado
      >
...
MenuItem.propTypes = {
  item: PropTypes.shape({
    ...
    subOptions: PropTypes.arrayOf(
      // Se houver subOpções, deve ser um array de objetos
      PropTypes.shape({
...
const Sidebar = ({ menuItems, setContent }) => {
  ...
  const renderedMenuItems = useMemo(
    ...
    [menuItems, activeIndex, handleSelect, openDropdownIndex]
  ); // O array de dependências garante que o menu seja re-renderizado apenas quando necessário
    ...
    <div>
      ...
      {showOverlay && (
        <div className="overlay" onClick={() => setShowOverlay(false)}>
          ...
          <div
            className="sidebar d-flex flex-column p-3 vh-100 shadow-sm"
...
Sidebar.propTypes = {
  menuItems: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      value: PropTypes.string, // O valor é opcional
      subOptions: PropTypes.arrayOf(
        // As subOpções são um array de objetos
        PropTypes.shape({
          label: PropTypes.string.isRequired, // O rótulo das subOpções é obrigatório
